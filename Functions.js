import * as dntShim from "./Deno2Node.js"; // Deno to Node.js compatibility layer.
...
const UPSTASH = {
    url: dntShim.Deno.env.get('UPSTASH_REDIS_REST_URL'),
    token: dntShim.Deno.env.get('UPSTASH_REDIS_REST_TOKEN'),
    GET: async (key) => {
...
export async function CreateJWT(username) {
    const privatePem = dntShim.Deno.env.get('JWT_PRIVATE_KEY_PEM').replace(/\\n/g, '\n'); // Replace literal backslashes with newlines.
    const jwtSecretKey = await crypto.subtle.importKey('pkcs8', pemToArrayBuffer(privatePem), { name: 'RSASSA-PKCS1-v1_5', hash: 'SHA-256' }, false, ['sign']);
    ...
    const signature = await crypto.subtle.sign({ name: 'RSASSA-PKCS1-v1_5' }, jwtSecretKey, signatureData);
    const encodedSignature = base64urlEncode(new Uint8Array(signature));
    return `${encodedHeader}.${encodedPayload}.${encodedSignature}`;
}
export async function VerifyJWT(token) {
    const publicPem = dntShim.Deno.env.get('JWT_PUBLIC_KEY_PEM').replace(/\\n/g, '\n'); // Replace literal backslashes with newlines.
    if (!publicPem)
        throw new Error('Missing JWT_PUBLIC_KEY_PEM');
    const publicKey = await crypto.subtle.importKey('spki', pemToArrayBuffer(publicPem), { name: 'RSASSA-PKCS1-v1_5', hash: 'SHA-256' }, false, ['verify']);
    try {
        ...
        const isValid = await crypto.subtle.verify({ name: 'RSASSA-PKCS1-v1_5' }, publicKey, signature, signatureData);
        if (!isValid)
            return null;
...
export async function LoginHandler(req) {
    ...
    try {
        ...
        if (dntShim.Deno.env.get('USERNAME') === username && dntShim.Deno.env.get('PASSWORD') === password) {
...
export async function FyersToken(req) {
    try {
        const appHash = await dntShim.Deno.env.get('FYERS_APP_HASH'); // The 'appID' and 'appSecret' are already Hashed and stored in environment variables.
