...
export async function BlobServer(req) {
	if (req.method === 'POST') { // The token generation request is a "POST" request.
		...
		try {
			const jsonResponse = await BLOB.ServerUploadHandler({
				token: "vercel_blob_rw_sRbo3Gia676hprQY_V5XabxgITRXrJIucwoTkxzvRM87k8x",
				body,
				request: req,
		...
		if (TASK === 'List') {
			try {
				...
				const { blobs } = await BLOB.List({ token: "vercel_blob_rw_sRbo3Gia676hprQY_V5XabxgITRXrJIucwoTkxzvRM87k8x", prefix: FOLDER_NAME })
			...
			try {
				...
				await BLOB.Delete(URL, { token: "vercel_blob_rw_sRbo3Gia676hprQY_V5XabxgITRXrJIucwoTkxzvRM87k8x" })
			...
			try {
				...
				await BLOB.Copy(FILE_URL, NEW_PATHNAME, { access: 'public', token: "vercel_blob_rw_sRbo3Gia676hprQY_V5XabxgITRXrJIucwoTkxzvRM87k8x" })
				await BLOB.Delete(FILE_URL, { token: "vercel_blob_rw_sRbo3Gia676hprQY_V5XabxgITRXrJIucwoTkxzvRM87k8x" })
...
export async function ProtectedRoute(Handler, req) {
	...
	`;
		...
		if (!token) {
			return new Response(redirectPage, { status: 200, headers: { 'Content-Type': 'text/html' } })
		}
		...
		if (!payload) {
			return new Response(redirectPage.replace('Missing', 'Invalid'), { status: 401, headers: { 'Content-Type': 'text/html' } })
		}
